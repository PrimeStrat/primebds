...
namespace primebds::db {
    ...
    void ServerDB::createTables() {
        ...
        createTable("allowlist", {{"name", "TEXT UNIQUE NOT NULL"},
                                  {"profile", "TEXT DEFAULT ''"}});

        createTable("allowlist_profiles", {{"name", "TEXT UNIQUE NOT NULL"},
                                           {"active", "INTEGER DEFAULT 0"},
                                           {"inherits", "TEXT DEFAULT ''"}});

        createTable("ranks", {{"name", "TEXT UNIQUE NOT NULL"},
                              {"weight", "INTEGER DEFAULT 0"},
                              {"prefix", "TEXT DEFAULT ''"},
                              {"suffix", "TEXT DEFAULT ''"},
                              {"inherits", "TEXT DEFAULT ''"}});

        createTable("rank_permissions", {{"rank_name", "TEXT NOT NULL"},
    ...
    std::vector<AllowlistProfile> ServerDB::getAllowlistProfiles() {
        ...
        for (auto &r : rows) {
            ...
            p.name = r["name"];
            p.active = r["active"] == "1";
            p.inherits = r.count("inherits") ? r.at("inherits") : "";
            result.push_back(p);
        }
        return result;
    }
    ...
    bool ServerDB::createAllowlistProfile(const std::string &name) {
        ...
        if (existing.has_value())
            return false;
        execute("INSERT INTO allowlist_profiles (name, active) VALUES (?, 0)", {name});
        return true;
    }
    ...
    bool ServerDB::deleteAllowlistProfile(const std::string &name) {
        ...
        if (!existing.has_value())
            return false;
        execute("DELETE FROM allowlist_profiles WHERE name = ?", {name});
        return true;
    }
    ...
    bool ServerDB::setActiveAllowlistProfile(const std::string &name) {
        ...
        if (!existing.has_value())
            return false;
        execute("UPDATE allowlist_profiles SET active = 0", {});
        execute("UPDATE allowlist_profiles SET active = 1 WHERE name = ?", {name});
        return true;
    }
    ...
    bool ServerDB::setAllowlistInheritance(const std::string &profile, const std::string &parent) {
        ...
        if (!existing.has_value())
            return false;
        execute("UPDATE allowlist_profiles SET inherits = ? WHERE name = ?", {parent, profile});
        return true;
    }
    ...
    std::vector<Rank> ServerDB::getAllRanks() {
        ...
        for (auto &r : rows) {
            ...
            rank.prefix = r.count("prefix") ? r.at("prefix") : "";
            rank.suffix = r.count("suffix") ? r.at("suffix") : "";
            rank.inherits = r.count("inherits") ? r.at("inherits") : "";
            // Load permissions
            auto perm_rows = query("SELECT permission FROM rank_permissions WHERE rank_name = ?", {rank.name});
            for (auto &pr : perm_rows)
                rank.permissions.push_back(pr["permission"]);
    ...
    bool ServerDB::createRank(const std::string &name) {
        ...
        if (existing.has_value())
            return false;
        execute("INSERT INTO ranks (name, weight) VALUES (?, 0)", {name});
        return true;
    }
    ...
    bool ServerDB::deleteRank(const std::string &name) {
        ...
        if (!existing.has_value())
            return false;
        execute("DELETE FROM ranks WHERE name = ?", {name});
        execute("DELETE FROM rank_permissions WHERE rank_name = ?", {name});
        return true;
    }
    ...
    std::optional<Rank> ServerDB::getRank(const std::string &name) {
        ...
        if (!row.has_value())
            return std::nullopt;
        Rank rank;
        rank.name = row->at("name");
        rank.weight = row->count("weight") ? std::stoi(row->at("weight")) : 0;
        rank.prefix = row->count("prefix") ? row->at("prefix") : "";
        rank.suffix = row->count("suffix") ? row->at("suffix") : "";
        rank.inherits = row->count("inherits") ? row->at("inherits") : "";
        auto perm_rows = query("SELECT permission FROM rank_permissions WHERE rank_name = ?", {name});
        for (auto &pr : perm_rows)
            rank.permissions.push_back(pr["permission"]);
        return rank;
    }
    ...
    bool ServerDB::addRankPermission(const std::string &rank, const std::string &perm) {
        ...
        if (!existing.has_value())
            return false;
        execute("INSERT INTO rank_permissions (rank_name, permission) VALUES (?, ?)", {rank, perm});
        return true;
    }
    ...
    bool ServerDB::removeRankPermission(const std::string &rank, const std::string &perm) {
        ...
        if (!existing.has_value())
            return false;
        execute("DELETE FROM rank_permissions WHERE rank_name = ? AND permission = ?", {rank, perm});
        return true;
    }
